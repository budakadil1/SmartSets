...
def search_sets(request):
    ...
    if request.user.is_authenticated:
        match_sets = Sets.objects.filter((Q(name__icontains=query) | Q(description__icontains=query)) & Q(public=True) | (Q(name__icontains=query) | Q(description__icontains=query)) & Q(author=request.user) | (Q(name__icontains=query) | Q(description__icontains=query)) & Q(shared_with=request.user))
    else:
